...
ml=c("IL1B","PTGS2","IL6","ACSL1","JUN","EGR1","TNFAIP3",
     "CDKN1A","GABARAPL1","ZFP36","SGK1","CXCL8","MMP9","CXCL1","NLRP3",
     "SLC2A3","TNF","IL1A","C5AR1","DDIT4"
)
...
pdf(file="Fig6A.boxplot.pdf", width=6, height=6)
...
pdf(file="Fig6B.ROC.pdf", width=8, height=8)
...
pdf(file="Fig6E.importance.pdf", width=7, height=10)
...
geneNum=5     
write.table(importance_rf[(ncol(data)-geneNum+2):(ncol(data)+1),], file="male.importanceGene.RF.txt", sep="\t", quote=F, row.names=F)
write.table(importance_svm[(ncol(data)-geneNum+2):(ncol(data)+1),], file="male.importanceGene.SVM.txt", sep="\t", quote=F, row.names=F)
write.table(importance_xgb[(ncol(data)-geneNum+2):(ncol(data)+1),], file="male.importanceGene.XGB.txt", sep="\t", quote=F, row.names=F)
write.table(importance_glm[(ncol(data)-geneNum+2):(ncol(data)+1),], file="male.importanceGene.GLM.txt", sep="\t", quote=F, row.names=F)
