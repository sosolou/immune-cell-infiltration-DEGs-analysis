rm(list = ls()) 
setwd("C:/Users/Administrator/Desktop/ischemic stroke & cell death/4.PRDEGs/female")

library(dplyr)
library(tidyverse)
pyrogene=read.table("pyrogene.txt", header=F, sep="\t", check.names=F) %>%
  distinct()
...
pyro.norm=norm[which(norm$Gene %in% pyrogene$V1),]  %>% as_tibble() %>%
  column_to_rownames("Gene")

library(limma)
library(pheatmap)
exp <- as.matrix(pyro.norm) 
...
pdf("Fig4K.GO.circlize.pdf",width=10,height=10)
...
pdf(file="Fig4G.vol.pdf", width=7, height=6.1)
print(p)
dev.off()


pyro.exp = diffGeneExp

pyro.t <- t(pyro.exp) 

library(reshape2)
exp=as.data.frame(pyro.t)
...
pdf(file="Fig4H.boxplot.pdf", width=7, height=5)
print(p1)
dev.off()

cutoff=0.1                     
rt=pyro.exp[,-1:-conNum]
data=t(rt)
cordata=cor(data)
...
pdf("Fig4J.co-expression network among differential pyro-genes.pdf", width=7, height=6)
...
pdf(file="Fig4I.RCircos.pdf", width=7, height=7)
RCircos.Set.Plot.Area()
RCircos.Chromosome.Ideogram.Plot()


RCircos.Gene.Label.Data=read.table("Rcircos.geneLabel.txt", header=T, sep="\t", check.names=F)
RCircos.Gene.Label.Data=RCircos.Gene.Label.Data[which(RCircos.Gene.Label.Data$Gene %in% colnames(pyro.t)),]
name.col <- 4
side <- "in"
track.num <- 1
...
group=gsub("(.*)\\_(.*)", "\\2", colnames(pyro.exp))
data=pyro.exp[,group=="patient",drop=F]
data=t(data)
...
pdf(file="Fig4L.immun-cor.pdf", width=7, height=5)
